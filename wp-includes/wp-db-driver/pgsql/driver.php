...
global $table_prefix;
$GLOBALS['pg4wp_wp_table_seqs'] = array(
    $table_prefix . 'commentmeta' => defined('PG4WP_SEQ_COMMENTMETA') ? PG4WP_SEQ_COMMENTMETA : '',
    $table_prefix . 'comments' => defined('PG4WP_SEQ_COMMENTS') ? PG4WP_SEQ_COMMENTS : '',
    $table_prefix . 'links' => defined('PG4WP_SEQ_LINKS') ? PG4WP_SEQ_LINKS : '',
    $table_prefix . 'options' => defined('PG4WP_SEQ_OPTIONS') ? PG4WP_SEQ_OPTIONS : '',
    $table_prefix . 'postmeta' => defined('PG4WP_SEQ_POSTMETA') ? PG4WP_SEQ_POSTMETA : '',
    $table_prefix . 'posts' => defined('PG4WP_SEQ_POSTS') ? PG4WP_SEQ_POSTS : '',
    $table_prefix . 'term_taxonomy' => defined('PG4WP_SEQ_TAXONOMY') ? PG4WP_SEQ_TAXONOMY : '',
    $table_prefix . 'terms' => defined('PG4WP_SEQ_TERMS') ? PG4WP_SEQ_TERMS : '',
    $table_prefix . 'termmeta' => defined('PG4WP_SEQ_TERMTERM') ? PG4WP_SEQ_TERMTERM : '',
    $table_prefix . 'usermeta' => defined('PG4WP_SEQ_USERMETA') ? PG4WP_SEQ_USERMETA : '',
...
function wpsql_close($conn) {
    return pg_close($conn);
}

function wpsql_server_version($conn) {
    $version = pg_version($conn);
    return $version['server'] ?? '9.6.30';
}

function wpsql_get_server_info($conn) {
    $version = pg_version($conn);
...
function wpsql_fetch_field($result, $index) {
    $info = array();
    $info['name'] = pg_field_name($result, $index);
    $info['table'] = pg_field_table($result, $index);
    $info['type'] = pg_field_type($result, $index);
    return $info;
}
...
function wpsql_affected_rows($conn = null) {
...
function wpsql_error($conn = null) {
    if (null != $conn) {
...
function wpsql_client_encoding($conn) {
...
function wpsql_query($sql, $conn) {
    $initial = $sql;
    $sql = pg4wp_rewrite($conn, $sql);
    
    $GLOBALS['pg4wp_result'] = pg_query($conn, $sql);
...
function pg4wp_table_seq($tablename) {
    $seq = '';
    if (isset($GLOBALS['pg4wp_wp_table_seqs'][$tablename])) {
        $seq = $GLOBALS['pg4wp_wp_table_seqs'][$tablename];
    }
    if (empty($seq)) {
        $seq = $tablename . '_seq';
    }
...
function pg4wp_rewrite($conn, $sql)
{
    global $wpdb;
    ...
    {
        require_once( PG4WP_ROOT.'/pgsql/install.php');
