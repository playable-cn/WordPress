...
require_once( PG4WP_ROOT . '/pgsql/driver.php' );
...
class wpdb {
	...
	/**
	 * PGSQL result, which is either a resource or boolean.
	 *
	 * @since 0.71
	 * @var mixed
	 */
	 ...
	 *
	 * @param string $dbuser     PGSQL database user
	 * @param string $dbpassword PGSQL database password
	 * @param string $dbname     PGSQL database name
	 * @param string $dbhost     PGSQL database host
	 * @param string $dbport     PGSQL database port
	 * @param string $dbschema   PGSQL database schema
	 */
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost, $dbport, $dbschema ) {
	...
	public function select( $db ) {
		$connectDsn = sprintf('host=%s port=%d dbname=%s user=%s password=%s', $this->dbhost, $this->dbport, $db, $this->dbuser, $this->dbpassword);

		$dbh = @pg_connect($connectDsn);
		if ( ! $dbh ) {
			$this->ready = false;
	...
	/**
	 * Real escape, using wpsql_real_escape_string()
	 *
	...
	/**
	 * First half of escaping for LIKE special characters % and _ before preparing for PGSQL.
	 *
	...
	public function db_connect( $allow_bail = true ) {
		$connectDsn = sprintf('host=%s port=%d dbname=%s user=%s password=%s', $this->dbhost, $this->dbport, $this->dbname, $this->dbuser, $this->dbpassword);
		if ( WP_DEBUG ) {
			$this->dbh = pg_connect($connectDsn);
		} else {
			$this->dbh = @pg_connect($connectDsn);
		}
	...
	public function check_connection( $allow_bail = true ) {
		if ( ! empty( $this->dbh ) && pg_ping( $this->dbh ) ) {
			return true;
		}
	...
	/**
	 * Perform a PGSQL database query, using current database connection.
	 *
	...
	public function query( $query ) {
		...
		// PGSQL server has gone away, try to reconnect.
		$wpsql_errno = 0;
		if ( ! empty( $this->dbh ) ) {
		...
		if ( is_resource( $this->dbh ) ) {
			...
			$this->last_error = __( 'Unable to retrieve the error message from PGSQL' );
		}
	...
	/**
	 * Whether PGSQL database is at least the required minimum version.
	 *
	 * @since 2.5.0
	 *
	 * @global string $wp_version
	 * @global string $required_pgsql_version
	 *
	...
	/**
	 * Retrieves the PGSQL server version.
	 *
	 * @since 2.7.0
	 *
	...
	public function db_version() {
		return wpsql_server_version( $this->dbh );
	}
}
